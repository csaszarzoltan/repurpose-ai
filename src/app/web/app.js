...
function card(project){const li=document.createElement('li');li.className='project-card';const title=document.createElement('h3');title.textContent=project.title;const meta=document.createElement('p');meta.className='meta';meta.textContent=`${project.target_formats.length} formats • Updated ${new Date(project.updated_at).toLocaleString()}`;const pill=document.createElement('span');pill.className='status-pill';pill.textContent=project.status;const archived=project.status==='archived';const generateButton=document.createElement('button');generateButton.type='button';generateButton.className='primary';generateButton.textContent='Generate drafts';generateButton.disabled=archived;generateButton.onclick=()=>generate(project);const view=document.createElement('button');view.type='button';view.textContent='View drafts';view.onclick=()=>showVariants(project.id);const edit=document.createElement('button');edit.type='button';edit.textContent='Edit project';edit.disabled=archived;edit.onclick=()=>startEdit(project);const duplicate=document.createElement('button');duplicate.type='button';duplicate.textContent='Create similar';duplicate.onclick=async()=>{const requested=window.prompt('New project title',`${project.title} copy`);if(requested===null)return;const response=await fetch(`/api/v1/projects/${project.id}/duplicate`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:requested.trim()||null})});if(!response.ok){say('Similar project could not be created.');return}const created=await response.json();say(`${created.title} created without copying drafts or publication state.`);await load();await loadSummary()};const archive=document.createElement('button');archive.type='button';archive.textContent='Archive';archive.setAttribute('aria-label',`Archive ${project.title}`);archive.onclick=async()=>{if(!window.confirm(`Archive ${project.title}? You can restore it from Archived projects.`))return;const response=await fetch(`/api/v1/projects/${project.id}`,{method:'DELETE'});if(!response.ok){say('Project could not be archived.');return}say(`${project.title} archived. It can be restored later.`);variantSection.hidden=true;load();loadSummary()};const restoreProject=document.createElement('button');restoreProject.type='button';restoreProject.textContent='Restore project';restoreProject.onclick=async()=>{const response=await fetch(`/api/v1/projects/${project.id}/restore`,{method:'POST'});if(!response.ok){say('Project could not be restored.');return}say(`${project.title} restored.`);await load();await loadSummary()};const actions=document.createElement('div');actions.className='actions';if(archived)actions.append(view,restoreProject);else actions.append(generateButton,view,edit,duplicate,archive);li.append(title,meta,pill,actions);return li}
let currentQuery='',showArchived=false;
async function load(){say('Loading projects…');try{const params=new URLSearchParams();if(currentQuery)params.set('q',currentQuery);if(showArchived)params.set('include_archived','true');const url=`/api/v1/projects${params.toString()?`?${params}`:''}`;const response=await fetch(url);if(!response.ok)throw new Error('Could not load projects');const projects=await response.json();list.replaceChildren(...projects.map(card));empty.hidden=projects.length>0;say(projects.length?`${projects.length} project${projects.length===1?'':'s'} loaded.`:'No saved projects.')}catch(error){say(`${error.message}. Try Refresh.`)}}
...
document.querySelector('#show-archived').addEventListener('change',event=>{showArchived=event.target.checked;load();say(showArchived?'Showing active and archived projects.':'Showing active projects only.')});
